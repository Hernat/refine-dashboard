...
export const getDate = (startDate: string, endDate: string) => {
    ...
    const start = dayjs(startDate).locale("fr").format("DD MMM YYYY - HH:mm");
    const end = dayjs(endDate).locale("fr").format("DD MMM YYYY - HH:mm");
...
const filterDeal = (deal?: DealAggregate) =>
    deal?.groupBy &&
    deal.groupBy?.closeDateMonth &&
    deal.groupBy?.closeDateYear;
